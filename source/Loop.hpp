...
//! \name Standard loop `typedef`s
//! \{
using AiLoop = Loop::Loop<   signed long >; //!< `signed long int` loop typedef
using AuLoop = Loop::Loop< unsigned long >; //!< `unsigned long int` loop typedef
using AfLoop = Loop::Loop<  float >; //!< `float` loop typedef
using AdLoop = Loop::Loop< double >; //!< `double` loop typedef
//! \}
